...
def check_issn_match(html, issns):
    if html is None:
        return False
    for issn in issns:
        issn_ = "[-–—]".join(issn.split("-")) # match different dash types, just in case
        if re.search(rf"{issn_}", html):
            return True
    return False
...
def process_row(row):
    ...
    if html is not None:
        html = unescape(html) # unescape HTML entities, for the dash in ISSNs
