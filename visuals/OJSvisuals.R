...
wbGroups <- read_excel("./data/wb_tlds.xlsx") %>% 
  ...
  mutate(
    income_group = if_else( #revert Venezuela's classification from
      #"Unclassified" to "Upper middle income"
...
beaconRegions <- merge(
  ...
  select(
    ...
    country,
    income_group) %>% 
  filter(!str_detect(country, "Kosovo")) %>% 
  mutate(
    un_region = case_when(
...
ggplot(beaconRegions,
       aes(y = 
             fct_rev(fct_infreq(un_region)) #forcats package
           )
       ) +
  theme_classic() +
  theme(
...
ggplot(beaconRegions,
       aes(y = 
             fct_rev(fct_infreq(income_group)) #forcats package
           )
       ) +
  theme_classic() +
  theme(
